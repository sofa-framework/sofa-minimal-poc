...
int BatchGUI::mainLoop()
{
    if (groot)
    {
        msg_info("BatchGUI") << "Computing " << nbIter << " iterations.";
        sofa::helper::AdvancedTimer::begin("Animate");
        sofa::simulation::getSimulation()->animate(groot.get());
        std::string out = sofa::helper::AdvancedTimer::end("Animate", groot.get());
        if(!out.empty())
        {
            msg_info("BatchGUI") << out;
        }
        ...
        for (unsigned int i=0; i<nbIter-1; i++) // one simulation step is animated above
        {
            sofa::helper::AdvancedTimer::begin("Animate");
            sofa::simulation::getSimulation()->animate(groot.get());
            out = sofa::helper::AdvancedTimer::end("Animate", groot.get());
            if(!out.empty())
            {
                msg_info("BatchGUI") << out;
            }
        ...
        msg_info("BatchGUI") << nbIter << " iterations done in " << ((double)t)/((double)tfreq) << " s ( " << (((double)tfreq)*nbIter)/((double)t) << " FPS).";
        msg_info("BatchGUI") << nbIter << " iterations done in " << ((double)rt)/((double)rtfreq) << " s ( " << (((double)rtfreq)*nbIter)/((double)rt) << " FPS).";
    }
    return 0;
}
...
BatchGUI* BatchGUI::CreateGUI(const char* /*name*/, sofa::simulation::Node::SPtr groot, const char* filename)
{
